...
function replaceOnce(path, source, oldValue, newValue) {
  if (!source.includes(oldValue)) {
    throw new Error(`Trecho não encontrado em ${path}: ${oldValue.slice(0, 160)}`);
  }
  return source.replace(oldValue, newValue);
}

write("backend/src/services/contasPagar/omieRequest.js", `"use strict";
...
write("backend/src/services/contasPagar/conclusaoRecebimento.js", `"use strict";
...
let operations = read("backend/src/services/contasPagar/omieOperations.js");
operations = operations.replace('const { array, primeiroValor } = require("./utils");', 'const { array, primeiroValor } = require("./utils");\nconst { executarChamadaOmie } = require("./omieRequest");');
operations = operations.replace(/async function executarChamadaOmie\([\s\S]*?\n}\n\nfunction cabecalhoRecebimento/, "function cabecalhoRecebimento");
operations = operations.replace(/async function listarRecebimentosOmie\([\s\S]*?\n}\n\nasync function enfileirarConclusaoCompras/, "async function enfileirarConclusaoCompras");
operations = operations.replace(/async function executarConclusaoRecebimentoOmie\([\s\S]*?\n}\n\nasync function executarEnvioContaPagarOmie/, "async function executarEnvioContaPagarOmie");
operations = operations.replace(/async function consultarPagamentoContaPagar\([\s\S]*?\n}\n\nasync function executarConsultaPagamentoOmie/, `async function consultarPagamentoContaPagar(contaOrId) {
...
write("backend/src/services/contasPagar/omieOperations.js", operations);

let indexSource = read("backend/src/services/contasPagar/index.js");
if (!indexSource.includes('...require("./omieRequest")')) {
  indexSource = indexSource.replace('  ...require("./omieOperations"),', '  ...require("./omieOperations"),\n  ...require("./omieRequest"),');
}
write("backend/src/services/contasPagar/index.js", indexSource);

write("backend/src/hooks/integrationAutoProcessor.js", `"use strict";
...
let mapping = read("backend/src/mappings/omie.js");
mapping = mapping.replace(/\n    "listar-recebimentos": \{[\s\S]*?\n    \},\n    "concluir-recebimento":/, '\n    "concluir-recebimento":');
mapping = mapping.replace(/param: \{ \$path: "\$input\.param", default: \[\{\}\] \}/g, 'param: { $path: "$input.param" }');
mapping = mapping.replace('      connectionTest: true\n', '      maxAttempts: 1,\n      connectionTest: true\n');
mapping = mapping.replace('      param: [{\n        pagina: "$input.page",\n        registros_por_pagina: "$input.pageSize"\n      }],\n      pagination:', '      param: [{\n        pagina: "$input.page",\n        registros_por_pagina: "$input.pageSize"\n      }],\n      maxAttempts: 1,\n      pagination:');
mapping = mapping.replace('        apenas_importado_api: "N"\n      }],\n      pagination:', '        apenas_importado_api: "N"\n      }],\n      maxAttempts: 1,\n      pagination:');
for (const call of ["incluir-conta-pagar", "alterar-conta-pagar", "consultar-conta-pagar", "excluir-conta-pagar", "concluir-recebimento"]) {
  const re = new RegExp(`("${call}": \\{[\\s\\S]*?param: \\{ \\$path: "\\$input\\.param" \\})(?!,\\n      maxAttempts)`);
  mapping = mapping.replace(re, '$1,\n      maxAttempts: 1');
}
mapping = mapping.replace('    webhookAction("Financas.ContaPagar.Excluido"),\n    webhookAction("*")', '    webhookAction("Financas.ContaPagar.Excluido")');
write("backend/src/mappings/omie.js", mapping);

const uiPath = "frontend/central.ui.json";
const ui = JSON.parse(read(uiPath));
ui.navigation = {
  mode: "mixed",
  items: [
    {
...
write(uiPath, `${JSON.stringify(ui, null, 2)}\n`);

let conclusionTest = read("backend/test/conclusaoRecebimento.test.js");
conclusionTest = conclusionTest.replace(/test\("a Central drena automaticamente a fila, webhooks e compras já pagas"[\s\S]*?\n}\);\n?$/, `test("a Central drena automaticamente somente tickets novos, sem reprocessar erros", () => {
...
`);
write("backend/test/conclusaoRecebimento.test.js", conclusionTest);

write("backend/test/consumoOmie.test.js", `"use strict";
...
// Falha cedo caso alguma transformação essencial não tenha ocorrido.
const finalMapping = read("backend/src/mappings/omie.js");
if (finalMapping.includes('"listar-recebimentos"') || finalMapping.includes("default: [{}]")) {
  throw new Error("Mapeamento Omie ainda contém pesquisa legada ou parâmetro vazio padrão.");
}
const callSection = finalMapping.slice(finalMapping.indexOf("calls: {"), finalMapping.indexOf("lists: ["));
const callCount = [...callSection.matchAll(/^    "[^"]+": \{/gm)].length;
const attemptCount = [...callSection.matchAll(/maxAttempts: 1/g)].length;
if (callCount !== attemptCount) throw new Error(`Nem todas as chamadas estão limitadas a uma tentativa: ${attemptCount}/${callCount}`);
